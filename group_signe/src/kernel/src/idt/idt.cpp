...
void start_idt() {
    ...
    //screenWrite("Loading IDT...");

    // Load the IDT
    //uint64_t* idt_ptr = reinterpret_cast<uint64_t*>(/* address of your IDT */);

    // Call the assembly function to load the IDT
    //loadIDT(idt_ptr);
    
    //idt_flush((uint32_t)&idt_ptr);

    // Adds feedback to the terminal
    //screenWrite("Completed Loading the IDT.");
